...
from .models import (
    ...
    images,
    user_liked,
    users_credentials_IPs,
    issues_raised,
    user_id_for_product,
    user_free_limits,
    send_text_to_owner,
    ad_posted_logs,
    ad_posting_payments_details,
    razorpay_payment_order_details,
    user_collections,
    unsafe_images,
    failure_response,
    super_Admin_users,
    users_credentials_app_token,
)
...
class ad_posting_payment(admin.ModelAdmin):
    list_display = ["plan_amount", "plan_id", "created_At"]
...
class user_info(admin.ModelAdmin):
    list_display = ["user_visited_ip_address", "visted_time"]
...
admin.site.register(users_credentials_app_token)
