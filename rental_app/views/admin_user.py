...
def admin_user(request):
    ...
    if cookies_token:
        ...
        validate = token_validations(cookies_token, request)
        ...
        if validate and str(validate[3]) == "admin":
            ...
            if user_fn:
                try:
                    ...
                    check_user_existance = users.objects.get(
                        Q(email_id=user_email) | Q(mobile_no=user_mobile)
                    )
                    ...
                    unused_images = (
                        (images.objects.filter(user_email_id=user_email))
                    ...
                    if unused_images:
                        try:
                            ...
                            shutil.rmtree(
                                f"media/{folder_id[0]}/user_posts/{folder_id[2]}"
                            )
                        ...
                        (images.objects.filter(user_email_id=user_email)).filter(
                            property_name="temp"
                        ).delete()
                    ...
                    try:
                        Previous_posted_info = (
                            property_details.objects.filter(
                                user_emailid_via_login_token=user_email
                            ).values(
                                "id",
                                "property_name",
                                "property_type",
            ...
            try:
                ...
                migrate_images_table = (
                    (images.objects.filter(user_email_id=new_user_email))
                    .filter(property_name="temp")
                    .update(property_name=new_data.id)
                )
    ...
    if current_user:
        ...
        users_previous_product_id = [
            ...
            for x in property_details.objects.filter(
                user_emailid_via_login_token=user_email_id
            ).values_list("id")
        ]
        ...
        if validate:
            new_images = images.objects.create(
                image=validate, user_email_id=user_email_id
            )
            new_images.save()
...
def validate_img(img, id):
    ...
    if user_side_img.format.upper() in allowed_formats:
        ...
        title_font = ImageFont.truetype(
            "rental_app/vermin_vibes_roundhouse.ttf", size=150
        )
        title_text = "RYS"
        image_to_edit = ImageDraw.Draw(user_side_img)
        image_to_edit.text(
            (user_side_img.width - 500, user_side_img.height - 200),
            title_text,
            (237, 230, 211),
            title_font,
        )
        ...
        try:
            ...
            pass
        user_side_img.save(
            f"media/{id}/{file_name}_{file_name1}.jpeg",
        )
